...
class Trainer():
    ...
    def train(self, model, loss, device, tr_loader, va_loader=None, optimizer=None, scheduler=None):
        ...
        filename = os.path.join(args.model_folder, 'final_model.pth')
        save_model(model, filename)
    ...
    def test(self, model, device, loader):
        ...
        std_loss /= iteration
        self.logger.info("Test accuracy {:.3f}%, test loss {:.3f}".format(std_acc, std_loss))
        return std_acc